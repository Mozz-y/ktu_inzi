  ...
  try {
    ...
    return cast
      ...
      .map((m: any) => ({
        ...
        searchedActorId: actorId,
        searchedActorName: people[0]?.name || actorName,
      }));
  } catch (error) {
...
export const actorsByMovie = async (movieId: number, includeActorId?: number) => {
  ...
  try {
    ...
    let visibleCast = cast.slice(0, 10);

    if (includeActorId) {
      const searchedActor = cast.find(
        (actor: any) => Number(actor?.id) === Number(includeActorId)
      );

      if (searchedActor && !visibleCast.some((actor: any) => Number(actor?.id) === Number(includeActorId))) {
        visibleCast = [...visibleCast.slice(0, 9), searchedActor];
      }
    }

    return visibleCast.map((actor: any) => {
